...
import {
  getJournalsMesOutS,
  getJournalsMesOutImport
} from "../services/journalService";
import JournalMesOutTable from "./journalMesOutTable";
import { paginate } from "../utils/paginate";
import _ from "lodash";
import SearchBox from "./common/searchBox";

class JournalMesOutS extends Component {
  state = {
  ...
  async componentDidMount() {
    ...
    const { data: journals } = await getJournalsMesOutS();
  ...
  async handleImport() {
    ...
    //holding result of async operations (that is going to complete in a future)
    const okNotOk = await getJournalsMesOutImport();
    console.log(okNotOk);
  ...
  getPagedData = () => {
    ...
    //TODO: can't lowercase of null
    if (searchQuery) {
      // const filteredNotNull = allJournals.filter(x => x.c_addrobjtl !== null);
      // console.log(filteredNotNull);
      filtered = allJournals.filter(p =>
        p.c_podpnam.toLowerCase().startsWith(searchQuery.toLowerCase())
      );
    }
      ...
      <div className="container-fluid">
        <div className="p-3 border bg-light">
          <div className="row">
            <div className="col-6">
              <h5>
                Журнал исходящие сообщения. Всего {totalCount}
                документов.
              </h5>
            </div>
